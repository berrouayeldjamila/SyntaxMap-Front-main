...
class PageQuestionMod extends React.Component {
  ...
  updateQuestion = e => {
    e.preventDefault();
    fetch(process.env.REACT_APP_API_URL + "/quiz/" + e.target[0].value,{
    method:'PUT',
    body: JSON.stringify({
  ...
  delete() {
        console.log(process.env.REACT_APP_API_URL + '/quiz/' + this.state.questionIdDelete);
        fetch(process.env.REACT_APP_API_URL + '/quiz/' + this.state.questionIdDelete,{
        method:'DELETE',
        headers: {
                "Content-type": "application/json; charset=UTF-8",
                "Authorization" : localStorage.getItem('jstoken')
