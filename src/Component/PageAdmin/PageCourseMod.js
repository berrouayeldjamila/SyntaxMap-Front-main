...
class PageCourseMod extends React.Component {
  ...
  updateCourse = e => {
    ...
    fetch(process.env.REACT_APP_API_URL + "/course/" + e.target[0].value,{
    method:'PUT',
    body: JSON.stringify({
  ...
  delete() {
        console.log(process.env.REACT_APP_API_URL + '/course/' + this.state.courseIdDelete);
        fetch(process.env.REACT_APP_API_URL + '/course/' + this.state.courseIdDelete,{
        method:'DELETE',
        headers: {
                "Content-type": "application/json; charset=UTF-8",
                "Authorization" : localStorage.getItem('jstoken')
